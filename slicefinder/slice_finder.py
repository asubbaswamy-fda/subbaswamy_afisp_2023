...
class SliceFinder:
    def __init__(self, model, data):
        ...
        if model is not None:
            self.metrics_all = self.evaluate_model(self.data)
            self.reference = (np.mean(self.metrics_all), np.std(self.metrics_all), len(self.metrics_all))
    ...
    def find_slice(self, k=50, epsilon=0.2, alpha=0.05, degree=3, risk_control=True, max_workers=1,
        ...
        print ('sorting')
        # originally sorted by s.size
        # Now sorting by s.effect_size for consistency. We are looking for high effect size slices
        # Even if they are potentially small.
        slices = sorted(slices, key=lambda s: s.effect_size, reverse=True)
        # for s in slices:
        #     print(f"Rule {s}; Effect size {s.effect_size}")

        # for timing purposes, making this optional
        if save_slices:
    ...
    def slicing(self, interval_slice=True):
        ...
        for col in X.columns:
            ...
            # Adarsh Note: This was in the original code, but it resulted in
            # skipping continuous features. Now, instead of being skipped, the default
            # behavior is to bin continuous features into 10 buckets.
#             if len(uniques) == n:
#                 # Skip ID li.ke col
#                 continue
            if len(uniques) > n/2.:
                ...
                for i in range(len(bin_edges)-1):
                    if interval_slice:
                        ...
                        # X < a
                        data_idx = X[ X[col] < bin_edges[i+1]].index
                        s = Slice({col:[[bin_edges[0],bin_edges[i+1]]]}, data_idx)
                        slices.append(s)
                        # X >= a
                        data_idx = X[ X[col] >= bin_edges[i+1]].index
                        if bin_edges[i+1] < bin_edges[-1]:
                            s = Slice({col:[[ bin_edges[i+1], bin_edges[-1]]]}, data_idx)
                            slices.append(s)
            else:
                for v in uniques:
    ...
    def evaluate_model(self, data, metric=log_loss, eval2=True):
        ''' evaluate model on a given data (X, y), example by example '''

        # TODO: refactor this. It gets called a lot and is very inefficient.
        # WE simply want this to return the samplewise loss
        if eval2:
            return(self.evaluate_model2(data))
        ...
        if metric == log_loss or metric == brier_score_loss:
    ...
    def evaluate_model2(self, data, metric=log_loss):
        X, y = copy.deepcopy(data[0]), copy.deepcopy(data[1])
        X['Label'] = y
        X = X.dropna()
        y = X['Label'].to_numpy()
    ...
    def filter_by_effect_size(self, slices, reference, epsilon=0.5, max_workers=1, alpha=0.05, risk_control=True):
        ...
        print(f"{len(slices)} slices in filter_by_effect_size")
